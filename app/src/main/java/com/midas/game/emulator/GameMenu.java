...
//        dialog = new Dialog(context, R.style.GameDialogTheme);
        ...
        RelativeLayout surroundContainer = new RelativeLayout(context);// = (RelativeLayout) inflater.inflate(R.layout.game_menu_surround, null);
        surroundContainer.setOnClickListener(v -> {
            if (dialog != null)
                dialog.cancel();
        });
        LinearLayout container = new LinearLayout(context);// = surroundContainer.findViewById(R.id.game_menu_container);
        RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) container.getLayoutParams();
        WindowManager wm = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
        Display display = wm.getDefaultDisplay();
        int width = 0;// = EmuUtils.getDisplayWidth(display);
        int px = width / 10;
        params.setMargins(px, 0, px, 0);
        container.setLayoutParams(params);
        int padding = 0;// = context.getResources().getDimensionPixelSize(
                //R.dimen.dialog_back_padding);
        container.setPadding(padding, padding, padding, padding);
        int margin = 0;// = context.getResources().getDimensionPixelSize(
